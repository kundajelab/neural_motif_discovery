...
def fetch_bigwig_paths(base_path, tf_name):
    ...
    `generate_ENCODE_replicate_bigwigs.sh`. This function performs
    error-checking to ensure that the proper files are there under `base_path`.
    Arguments:
        `base_path`: path containing the BigWig profiles of replicates
        `tf_name`: name of TF
...
def create_hdf5(
    ...
    matrix of the read counts at that position, for the two replicates and
...
@click.option(
    ...
    help="Path to directory containing BigWigs; defaults to /users/amtseng/tfmodisco/data/interim/ENCODE/{tf_name}/tf_chipseq_replicate_bigwigs/"
)
@click.option(
    "--chrom-sizes-path", "-c",
...
@click.option(
    ...
    help="Destination for new HDF5; defaults to /users/amtseng/tfmodisco/data/processed/ENCODE/labels/{tf_name}/{tf_name}_replicate_profiles.h5"
)
@click.option(
...
def main(tf_name, base_path, chrom_sizes_path, out_path, chunk_size):
    ...
    """
    if not base_path: 
        base_path = "/users/amtseng/tfmodisco/data/interim/ENCODE/%s/tf_chipseq_replicate_bigwigs/" % tf_name
    if not out_path:
        out_path = \
            "/users/amtseng/tfmodisco/data/processed/ENCODE/labels/%s/%s_replicate_profiles.h5" % (tf_name, tf_name)
