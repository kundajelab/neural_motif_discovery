...
job_dir = "/users/amtseng/tfmodisco/infra/gcp/jobs/pred_perf/"
specs_dir = os.path.join(job_dir, "specs")
job_template = os.path.join(job_dir, "pred_perf_template.yaml")
...
def create_tf_job_specs(
    ...
    """
    Creates separate job files that runs predictions/performance computation for
    all 10 folds on the TF, for the multi-task profile model and all single-task
    profile models, as well as the fine-tuned models.
    ...
    out_base = "/users/amtseng/tfmodisco/results/peak_pred_perf/"
    tf_lower = tf.lower()
    ...
    for fold_num, run_num, epoch_num in multitask_defs:
        ...
        out_path = os.path.join(
            ...
            "multitask_profile",
            "%s_multitask_profile_fold%d" % (tf, fold_num),
            "%s_multitask_profile_fold%d_pred_perf.h5" % (tf, fold_num)
        )
    ...
    for task_index, fold_num, run_num, epoch_num in singletask_defs:
        ...
        out_path = os.path.join(
            ...
            "%s_singletask_profile_fold%d" % (tf, fold_num),
            "task_%d" % task_index,
            "%s_singletask_profile_task%d_fold%d_pred_perf.h5" % (tf, task_index, fold_num)
        )
        filled = template.format(
    ...
    out_path = os.path.join(
        ...
        "multitask_profile_finetune",
        "%s_multitask_profile_finetune_fold%d" % (tf, fold_num),
        "%s_multitask_profile_finetune_fold%d_pred_perf.h5" % (tf, fold_num)
    )
    filled = template.format(
    ...
    for task_index, fold_num, run_num, epoch_num in finetune_singletask_defs:
        ...
        out_path = os.path.join(
            ...
            "%s_singletask_profile_finetune_fold%d" % (tf, fold_num),
            "task_%d" % task_index,
            "%s_singletask_profile_finetune_task%d_fold%d_pred_perf.h5" % (tf, task_index, fold_num)
        )
        filled = template.format(
...
def main(
    ...
    """
    Generates and launches Kubernetes jobs to run model predictions for a
    specific TF. This will run the predictions for the multi-task model and the
