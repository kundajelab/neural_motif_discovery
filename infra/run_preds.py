...
def main(tf_name, fold_num, run_num, epoch_num, num_tasks):
    ...
    out_path = os.path.join(
        "/users/amtseng/tfmodisco/results/peak_predictions/",
        tf_name,
        "%s_peak_prediction_performance_fold%d.h5" % (tf_name, fold_num)
    )
    ...
    comm = ["python", "-m", "extract.predict_peaks", "run"]
    comm += ["with"]
    comm += ["model_path=%s" % model_path]
    comm += ["files_spec_path=%s" % file_specs_json_path]
    comm += ["num_tasks=%d" % num_tasks]
    comm += ["out_hdf5_path=%s" % out_path]
